...
import ScrapperScripts.HelperScrapper as HS
...
for f1ManagerDriver in AS.F1Manager22RatingsData:
    ...
    DatabaseUpdate.f1ManagerUpdate(databaseCursor, str(f1ManagerDriver[0]), str(
        f1ManagerDriver[1]), int(f1ManagerDriver[3]))
    Database.DATABASECONNECTION.commit()
...
# Manually put some countries into the countries table
DatabaseUpdate.countriesUpdate(databaseCursor, "Thailand")
Database.DATABASECONNECTION.commit()
DatabaseUpdate.countriesUpdate(databaseCursor, "Denmark")
Database.DATABASECONNECTION.commit()
DatabaseUpdate.countriesUpdate(databaseCursor, "Finland")
Database.DATABASECONNECTION.commit()


# Put country into driver and teams table
for driver in SS.currentDriverNames:
    driverCountry = HS.getDriverCountry(driver)
    DatabaseUpdate.driverCountryUpdate(databaseCursor, driver, driverCountry)
    Database.DATABASECONNECTION.commit()
for team in SS.f1ConstructorsData:
    teamCountry = HS.getTeamCountry(team[0])
    DatabaseUpdate.teamCountryUpdate(databaseCursor, team[0], teamCountry)
    Database.DATABASECONNECTION.commit()


# Put weather id into country table
databaseCursor.execute('''SELECT fullName FROM countries;''',)
for country in databaseCursor.fetchall():
    weatherID = HS.getCountryWeatherID(str(country[0]))
    DatabaseUpdate.countryWeatherUpdate(
        databaseCursor, str(country[0]), int(weatherID))
    Database.DATABASECONNECTION.commit()
